...
export async function fetchShodanDevices() {
    ...
    try {
        ...
        const extractedDevices = data.matches
            .map((match: any) => ({
                ip: match.ip_str,
            ...
            .filter((device: ShodanDevice) => {
                ...
                return bacnetFields.some((field) => device[field as keyof ShodanDevice] !== "NULL");
            });

        return extractedDevices;
    } catch (error: any) {
        console.error(
            `[${new Date().toISOString()}] Error in fetchShodanDevices: ${error.message}`
        );
...
export async function refreshShodanDevices() {
    "use server";
    console.log(`[${new Date().toISOString()}] Refreshing cached Shodan devices`);
    try {
        revalidateTag("shodan-devices");
    } catch (error: any) {
        console.error(
            `[${new Date().toISOString()}] Error revalidating 'shodan-devices' tag: ${error.message}`
        );
