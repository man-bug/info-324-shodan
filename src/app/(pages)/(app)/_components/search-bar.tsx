...
export default function SearchBar({ initialDevices }: { initialDevices: ShodanDevice[] }) {
    ...
    return (
        <>
            {isSearching && (
                <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-75 z-20" />
            )}
            <div
                className={cn(
            ...
            >
                ...
                />
                <kbd className="pointer-events-none absolute right-8 top-1/2 -translate-y-1/2 h-5 select-none items-center gap-1 rounded-md border bg-secondary/50 px-1.5 text-[11px] font-medium opacity-100 sm:flex">
                    CTRL + K
                </kbd>
                <AnimatePresence>
                    {isSearching && (
                                ...
                                {searchQuery ? (
                                    filteredDevices.map((result, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <Button className="h-full w-full shadow-none normal-case rounded-none !bg-transparent text-foreground/80 hover:text-foreground px-4 py-2 text-left justify-between">
                                                    <p>
                                                        {highlightMatch(
                                                            result.modelName,
                                                            searchQuery
